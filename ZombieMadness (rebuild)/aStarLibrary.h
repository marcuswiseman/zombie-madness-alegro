...
//Declare constants
const int mapWidth = 200, mapHeight = 200, numberPeople = 3;
int onClosedList = 10;
...
//-----------------------------------------------------------------------------
int FindPath (int pathfinderID, int walkability[mapWidth][mapHeight], int startingX, int startingY,
        ...
        if (pathStatus[ID] == found) {
            ...
            if (pathLocation[ID] < pathLength[ID]) {
                ...
                if (pathLocation[ID] == 0 ||
                        ((currentX - xPath[ID]) < pixelsPerFrame && (currentY - yPath[ID]) < pixelsPerFrame))
            ...
            //reached then reset.
            if (pathLocation[ID] == pathLength[ID]) {
                if ((currentX - xPath[ID]) < pixelsPerFrame
                        && (currentY - yPath[ID]) < pixelsPerFrame) //if close enough to center of square
