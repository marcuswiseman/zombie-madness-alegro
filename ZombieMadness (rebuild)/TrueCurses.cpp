...
bool initscr(int height, int width,int color_depth,int font_scale_factor) {
    allegro_init();
    install_keyboard();
    ...
    if (set_gfx_mode(GFX_AUTODETECT_WINDOWED, width, height, 0, 0) == false) {
        return false;
    } else {
        return true;
    }
...
int basic_movement_x(char Map[MAPY][MAPX],int y0,int x0,PASSABLE pass[100]) {
    for (int i = 0; i < 100; i++) {
        if (key[KEY_RIGHT]) {
            if (mvgetch(Map,y0,x0+1) == pass[i].c) {
                return x0+1;
            }
        }
        if (key[KEY_LEFT]) {
...
int basic_movement_y(char Map[MAPY][MAPX],int y0,int x0,PASSABLE pass[100]) {
    for (int i = 0; i < 100; i++) {
        if (key[KEY_DOWN]) {
            if (mvgetch(Map,y0+1,x0) == pass[i].c) {
                return y0+1;
            }
        }
        if (key[KEY_UP]) {
...
void mvprintwx(int y, int x,string c,int foreground_color,int background_color) { // add string at position , set color
    textout_ex(buffer, font, c.c_str(), x*scr.font_scale_factor, y*scr.font_scale_factor, foreground_color, background_color);
}
